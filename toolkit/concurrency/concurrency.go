...
//
// Each index is either executed or, if ctx is done before the task starts,
// reported as ctx.Err(). A nil error means the task started and returned nil —
// never "the task was skipped".
func Pool(ctx context.Context, workers int, tasks []func(context.Context) error) []error {
		...
		go func() {
			...
			for idx := range jobs {
				...
				func(i int) {
					...
					atomic.StoreUint32(&started[i], 1)
					errs[i] = tasks[i](ctx)
				}(idx)
			}
		}()
	...
	if err := ctx.Err(); err != nil {
		for i := range tasks {
			if atomic.LoadUint32(&started[i]) == 0 && errs[i] == nil {
				errs[i] = err
			}
		}
	}
