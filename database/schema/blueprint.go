...
func (b *Builder) Rename(from, to string) error {
	...
	case "mssql", "sqlserver":
		...
		return err
	case "oracle", "ora":
		_, err := b.db.Exec(fmt.Sprintf("RENAME %s TO %s", from, to))
		return err
...
func (b *Builder) HasTable(table string) (bool, error) {
	...
	case "oracle", "ora":
		query = `SELECT COUNT(*) FROM user_tables WHERE table_name = UPPER(:1)`
	default:
...
func (b *Builder) HasColumn(table, column string) (bool, error) {
	...
	case "oracle", "ora":
		query = `SELECT COUNT(*) FROM user_tab_columns WHERE table_name = UPPER(:1) AND column_name = UPPER(:2)`
	default:
		return false, fmt.Errorf("unsupported driver: %s", b.driver)
	}
	var count int
...
func (b *Blueprint) columnSQL(col *Column) (string, error) {
	...
	if col.AutoIncrement {
		...
		case "oracle", "ora":
			// GENERATED AS IDENTITY handled in typeSQL
		}
	}
...
func (b *Blueprint) typeSQL(col *Column) (string, error) {
	...
	case "id":
		...
		case "oracle", "ora":
			return "NUMBER GENERATED BY DEFAULT AS IDENTITY", nil
		default:
			return "INTEGER", nil
		}
	case "foreignid":
	...
	case "string":
		switch b.driver {
		case "oracle", "ora":
			return fmt.Sprintf("VARCHAR2(%d)", col.Length), nil
		default:
